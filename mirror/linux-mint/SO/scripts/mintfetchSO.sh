...
MIRROR_DIRECTORY_SO=${MIRROR_DIRECTORY}/SO
...
function get_local_time() {
    if [ -f ${MIRROR_DIRECTORY}/SO.meta ]; then
        cat ${MIRROR_DIRECTORY}/SO.meta | head -n 1
    else
        echo "1970-01-01 00:00:00 UTC" >${MIRROR_DIRECTORY}/SO.meta
        echo "1970-01-01 00:00:00 UTC"
    fi
}
...
if [ $(should_pull "$(get_local_time)" "$(get_upstream_time)" "$(date -u)") == "true" ]; then
    ...
    sed -i "1s/.*/$new_date/" ${MIRROR_DIRECTORY}/SO.meta
    else
